...
// Raw GPU data from API (different field names)
interface GpuInfoRaw {
  node: string;
  index: string;
  name: string;
  temperature?: number;
...
// Transform raw GPU data to expected format
function transformGpuData(raw: GpuInfoRaw): GpuInfo {
  return {
    node: raw.node,
    index: raw.index,
    name: raw.name,
    temp_c: raw.temp_c ?? raw.temperature ?? 0,
    utilization: raw.utilization ?? 0,
    power_w: raw.power_w ?? raw.power_watts ?? 0,
    memory_used_gb: raw.memory_used_gb ?? (raw.memory_used_mb ? raw.memory_used_mb / 1024 : undefined),
    memory_total_gb: raw.memory_total_gb ?? (raw.memory_total_mb ? raw.memory_total_mb / 1024 : undefined),
  };
}
...
export const api = {
  ...
  gpuStatus: async () => {
    const raw = await fetchJSON<{ gpus: GpuInfoRaw[] }>('/gpu/status');
    return { gpus: raw.gpus.map(transformGpuData) };
  },
