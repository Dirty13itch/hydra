        ...
        <div className="flex-1 min-h-0 relative overflow-y-auto">
          {activeTab === 'OVERVIEW' && (
            <div className="h-full flex flex-col gap-4">
              {/* Real Thinking Stream from API */}
              {(agent as any).thinkingStream && (agent as any).thinkingStream.length > 0 ? (
                <ThinkingStream
                  steps={(agent as any).thinkingStream}
              ...
              {/* Legacy Log Stream (simulation) */}
              <Card className="flex-1 border-neutral-800 flex flex-col bg-neutral-900/50 p-0 overflow-hidden">
          ...
          {activeTab === 'HISTORY' && (
             <div className="space-y-4 animate-slide-in">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                   <Card className="bg-surface-dim">
                      <div className="flex items-center gap-3">
                         <div className="p-2 bg-emerald-500/10 rounded-lg text-emerald-500">
                            <CheckCircle2 size={18} />
                         </div>
                         <div>
                            <p className="text-xs text-neutral-500 font-mono">SUCCESS</p>
                            <p className="text-lg font-bold text-neutral-200">
                               {agent.actionHistory?.filter(a => a.status === 'success').length || 0}
                            </p>
                         </div>
                      </div>
                   </Card>
                   <Card className="bg-surface-dim">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <p className="text-xs text-neutral-500 font-mono">FAILED</p>
                            <p className="text-lg font-bold text-neutral-200">
                               {agent.actionHistory?.filter(a => a.status === 'failed').length || 0}
                            </p>
                         </div>
                      </div>
                   </Card>
                   <Card className="bg-surface-dim">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <p className="text-xs text-neutral-500 font-mono">TOKENS</p>
                            <p className="text-lg font-bold text-neutral-200">
                               {agent.actionHistory?.reduce((sum, a) => sum + (a.tokensUsed || 0), 0).toLocaleString() || 0}
                            </p>
                         </div>
                      </div>
                   </Card>
                ...
                <Card className="overflow-hidden">
                   <div className="p-4 border-b border-neutral-800 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-neutral-400">
                         <History size={16} />
                         <span className="text-sm font-mono uppercase">Action Log</span>
                      </div>
                      <Badge variant="neutral">{agent.actionHistory?.length || 0} actions</Badge>
                   </div>
                   <div className="divide-y divide-neutral-800 max-h-[400px] overflow-y-auto">
                      {agent.actionHistory?.map((action) => (
                         <div key={action.id} className="p-4 hover:bg-surface-raised transition-colors">
                            <div className="flex items-start gap-4">
                               ...
                               <div className={`mt-1 p-1.5 rounded ${
                                  ...
                                  action.status === 'partial' ? 'bg-amber-500/10 text-amber-500' :
                                  'bg-neutral-500/10 text-neutral-500'
                               }`}>
                                  {action.status === 'success' && <CheckCircle2 size={14} />}
                                  {action.status === 'failed' && <XOctagon size={14} />}
                                  {action.status === 'partial' && <AlertTriangle size={14} />}
                                  {action.status === 'cancelled' && <XCircle size={14} />}
                               </div>

                               {/* Content */}
                               <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 mb-1">
                                     <span className="font-mono text-sm text-neutral-200 uppercase">{action.action.replace('_', ' ')}</span>
                                     <Badge variant={action.status === 'success' ? 'emerald' : action.status === 'failed' ? 'red' : 'amber'}>
                                        {action.status}
                                     </Badge>
                                  </div>
                                  <p className="text-sm text-neutral-400 mb-2">{action.description}</p>

                                  {/* Tools Used */}
                                  {action.toolsUsed && action.toolsUsed.length > 0 && (
                                     <div className="flex flex-wrap gap-1 mb-2">
                                        {action.toolsUsed.map((tool, idx) => (
                                           <span key={idx} className="text-[10px] px-1.5 py-0.5 bg-neutral-800 text-neutral-400 rounded font-mono">
                                              {tool}
                                           </span>
                                        ))}
                                     </div>
                                  ...
                                  {/* Error Output */}
                                  {action.output && action.status !== 'success' && (
                                     <div className="text-xs text-red-400 bg-red-500/10 px-2 py-1 rounded mt-1 font-mono">
                                        {action.output}
                                     </div>
                                  )}
                               </div>
                               ...
                               <div className="text-right shrink-0 hidden md:block">
                                  ...
                                  {action.tokensUsed && (
                                     <div className="flex items-center gap-1 text-xs text-neutral-500 mb-1">
                                        <Zap size={10} />
                                        <span>{action.tokensUsed.toLocaleString()} tok</span>
                                     </div>
                                  )}
                      ...
                      {(!agent.actionHistory || agent.actionHistory.length === 0) && (
                         <div className="p-8 text-center text-neutral-600">
                            ...
                            <p>No action history recorded yet.</p>
                         </div>
                      )}
                   </div>
                </Card>
