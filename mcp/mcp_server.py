...
async def services_detailed():
    """Check status of key Hydra services with uptime info"""
    ...
    # Get container info via Docker API (once for all containers)
    container_started = {}
    try:
        ...
        async with httpx.AsyncClient(transport=transport) as docker:
            for service_name, container_name in service_containers.items():
                try:
                    r = await docker.get(f"http://localhost/containers/{container_name}/json")
                    if r.status_code == 200:
                        data = r.json()
                        started_at = data.get("State", {}).get("StartedAt", "")
                        if started_at:
                            # Parse ISO format: "2024-12-10T20:03:45.123456789Z"
                            # Remove nanoseconds but keep Z for timezone
                            if '.' in started_at:
                                started_str = started_at.split('.')[0] + 'Z'
                            else:
                                started_str = started_at
                            started = datetime.fromisoformat(started_str.replace('Z', '+00:00'))
                            now = datetime.now(timezone.utc)
                            uptime = int((now - started).total_seconds())
                            container_started[service_name] = uptime
                except:
                    pass
    except:
        pass
    ...
    for name, url, headers in checks:
        try:
            r = await client.get(url, timeout=5.0, headers=headers)
            status = "up" if r.status_code in [200, 204] else f"status:{r.status_code}"
        except:
            status = "down"

        services[name] = {
            "status": status,
            "uptime_seconds": container_started.get(name)
        }
