...
export default function Home() {
  ...
  const getNodeMetrics = (name: string): { name: string; cpu_pct?: number; memory_pct?: number } | null => {
    const entries = Object.entries(nodes);
    const match = entries.find(([instance, data]) => { if (data.name === name) return true; if (name === "hydra-storage" && data.name === "Unraid") return true; return instance.includes(name) || instance.includes(name.split('-')[1]));
